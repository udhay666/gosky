$(function() {
	function formatF(item) {
	    var cell = '';   
	    cell += "<span class='p-2'><i class='mdi mdi-car'></i></span><span class='p-2'>" + item.label + "</span>"
	    return cell;
	}
	...
	$("#pickupdroptype").catcomplete({ 
	    delay: 0,
	    // source: availableTags,
	    source: siteUrl+"home/car_city_dropofflist",   
	    minLength: 0,
	    autoFocus: true,
	    scroll: true,
	    select: function( event, ui ) {
	      if(ui.item.id == ''){
	        $(this).val('');
	        $('#pickupdropofftype').val('');
	        return false;
	      }else{
	      	$('#pickupdropofftype').val(ui.item.id);
	      }
	    },
	...
	$("body").on("focus","#pickupstarttype",function(){
	    // $(this).select();
