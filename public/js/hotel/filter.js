var siteUrl="https://www.tripfreebuy.com/";
...
function callPriceSlider(setPriceMin,setPriceMax,currency) {
    ...
    $selector.slider ({
        ...
        slide: function(event, ui) {
            if(ui.values[0]+20>=ui.values[1]) {
                ...
                $MinOutput.html('<i class="mdi mdi-currency-inr"></i> '+ ui.values[ 0 ].toLocaleString());                
                $MaxOutput.html("<i class='mdi mdi-currency-inr'></i> "+ui.values[ 1 ].toLocaleString());
    ...
    $MinOutput.html('<i class="mdi mdi-currency-inr"></i> '+$selector.slider( "values", 0 ).toLocaleString());    
    $MaxOutput.html("<i class='mdi mdi-currency-inr'></i> "+ $selector.slider( "values",1).toLocaleString());
...
function filter($sortBy,$order) {
    ...
    $.post(siteUrl+"hotels/searchresult_ajax", { sessionId: ""+$sessionId, minPrice: ""+$minPr, maxPrice: ""+$maxPr,tbolistc: ""+$locatedShow, starRating: ""+$starShow, hotelName: ""+$hotelName, location: ""+$locationShow, sortBy: ""+$sort_data, order: ""+$sort_order, searcharray: ""+$searcharray},
    function(data){
        $('.ajaxloading').hide();
...
function filter_all($sortBy,$order) {
    ...
    $searcharray=$('#searcharray').val();
    $.post(
        siteUrl+"hotels/searchresult_ajax",
        {
            sessionId: ""+$sessionId,
            minPrice: ""+$minPr,
            maxPrice: ""+$maxPr,
