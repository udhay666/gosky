$(document).ready(function()
    {	
        ...
        $('#result').html(data.activity_search_result);
        ...
        var prices = $(".ActivityInfoBox").map(function() {
        ...
        var durations = $(".ActivityInfoBox").map(function() {
     ...
     // console.log(VehicleString);
        ...
        for (var i = 0, l = api_array.length; i < l; i++) { //alert(i);

            $.ajax({
                url: siteUrl+'activity/activity_availability',				
                ...
                $.ajax({
                    url: siteUrl+'activity/fetch_results', 
...
// flight details open					
$(document).on("click", '.results-row > .ActvityInfoBox > .row', function($e){				
