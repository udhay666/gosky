$(document).on('click', '.btn-number', function(e){
    ...
    var pax_drop_parent = $(this).closest('.pax_drop');
    var htmldisp = pax_drop_parent.find("."+fieldName+"F");

    // var travellerF;
    // if(pax_drop_parent.hasClass('flight')){
        travellerF = pax_drop_parent.find(".travellerF");
    // }
    var pax_total = parseInt(travellerF.html());

    var currentVal = parseInt(input.val());

    var adultsval,childsval,infantsval,totPax;
    
    // console.log(pax_total);
    if (!isNaN(currentVal)) {
        if(type == 'minus') {
            if(currentVal > input.attr('min')) {
                ...
                infantsval = parseInt(pax_drop_parent.find("input[name='infants']").val());
                if(fieldName == 'adults' && infantsval >= currentVal){
                    pax_drop_parent.find("input[name='infants']").val(currentVal - 1);
                    pax_drop_parent.find('.btn-number.quantity-right-plus.chd').removeAttr('disabled');
                }
                if(fieldName == 'adults' && pax_total <= 9){
                    pax_drop_parent.find('.btn-number.quantity-right-plus.chd').removeAttr('disabled');
                }
                if(fieldName == 'childs'){
                    pax_drop_parent.find('.btn-number.quantity-right-plus.adt').removeAttr('disabled');
                }
                pax_total -= 1;
            }
        ...
        // console.log(pax_total);
        // if(pax_drop_parent.hasClass('flight')){
        adultsval = parseInt(pax_drop_parent.find("input[name='adults']").val());
        childsval = parseInt(pax_drop_parent.find("input[name='childs']").val());
        infantsval = parseInt(pax_drop_parent.find("input[name='infants']").val());
        totPax = adultsval + childsval + infantsval;

        if(totPax > 8) {
            pax_drop_parent.find('.btn-number.quantity-right-plus').attr('disabled', true);
        } else if(infantsval >= adultsval) {
            pax_drop_parent.find('.btn-number.quantity-right-plus.inf').attr('disabled', true);
        } else {
            pax_drop_parent.find('.btn-number.quantity-right-plus').removeAttr('disabled');
        }
        // }

        // travellerF.html(pax_total);
        travellerF.html(totPax);
    } else {
        input.val(0);
    }
});
...
$(document).on('click', '.travellers-input,.travellers-icon', function() {
...
$(document).mouseup(function(e) {
    var container = $(".travellers-input-popup");
    var container2 = $(".travellers-input");
