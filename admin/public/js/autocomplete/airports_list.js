$(function() {
	...
	$( "#fromCity" ).autocomplete({	
		...
		change: function(event, ui) {
            ...
            $( "#toCity" ).autocomplete({			
		source: siteUrl+"/home/airport_autolist?exclude="+$("input[name='origin']").val(),
		minLength: 2,
		autoFocus: true,               
		select: function( event, ui ) {
			if( ui.item.id == '' ) return false;
			$("input[name='destination']").val(ui.item.id);
			$( "#fromCity" ).autocomplete( "option", "source", siteUrl+"/home/airport_autolist?exclude="+$("input[name='destination']").val() );
