...
//echo $this->db->last_query();exit;
    ...
    public function upload_images_itinerary($id,$img,$img_type){
        $data=array(
            //'itinerary_id'=>$id,
            'itineraryimage'=>$img
        );
        $this->db->where('itinerary_id', $id);
        $this->db->update('holiday_itinerary_daywise',$data);
        return true;
    }


    public function upload_images_cabitinerary($id,$img,$img_type){
        $data=array(
           // 'itinerary_id'=>$id,
            'itinerarycab_image'=>$img
        );
        $this->db->where('itinerary_id', $id);
        $this->db->update('holiday_itinerary_daywise',$data);
        return true;
...
public function upload_package_home_category_theme_images($id,$home_category_image)
    ...
    public function holi_booking_report_details($first_name=null,$last_name=null,$uniqueRefNo=null,$user_mobile=null,$from_date=null,$to_date=null,$package_title=null,$user_email=null,$booking_status=null,$package_code=null,$assignto=null,$promo_code=null)
    {
       $this->db->select('*');
       $this->db->from('holiday_booking_reports');
       if($package_title !=''){
 ...
 public function holi_pre_booking_report_details($first_name='',$last_name='',$uniqueRefNo='',$user_mobile='',$from_date='',$to_date='',$package_title='',$user_email='',$booking_status='',$package_code='')
   ...
   public function holiday_booking_payment_report($status='',$contact='',$email='',$razorpay_id='',$paid_amount='',$uniqueRefNo='',$card_id='',$from_date='',$to_date='')
      ...
      public function getholivisitcity($cityid)
    {
     ...
     public function update_holiday_enquiry_status($data,$id)
    {
        $this->db->where('id',$id);
        $this->db->update('holiday_pac_req', $data);
        // $this->db->where('holiday_enquiry_id',$id);
        // $this->db->update('holiday_enquiry', $data);
    }
    public function get_holiday_pre_enquiry()
    {
        $this->db->select('*');
        $this->db->where('status',1);
        $this->db->from('holiday_pac_req');
    ...
    public function get_holiday_enquiry_by_id($id)
    {
        $this->db->select('*');
        $this->db->where('id', $id);
        $this->db->from('holiday_pac_req');
     ...
     public function add_booking_reports($data) {
