...
class Multifile_Upload extends CI_Upload {
	...
	const Org_Folder = 'org';
	const File_Prefix = '150';
	private $rename;
	private $resize;
	private $image_sizes;	
	public  $saved_filename = array();
	public function initialize($props = array())
	{
		$this->saved_filename = array();
		if (isset($props["rename"])) {
           $this->rename  = $props["rename"];
        }
        if (isset($props["resize"])) {
            $this->resize = $props["resize"];
        }
        if (isset($props["image_sizes"])) {
            $this->image_sizes = $props["image_sizes"];
        }		
	...
	// Is $_FILES[$field] set? If not, no reason to continue.
		...
		if($this->rename) {
			$this->file_name  =  self::File_Prefix . time() . $this->file_ext;
		}
		 ...
		 */
		if($this->resize) {
			$upload_path = $this->upload_path . self::Org_Folder . '/';
		} else {
			$upload_path = $this->upload_path;
		}		
		if ( ! @copy($this->file_temp, $upload_path.$this->file_name))
		{
			if ( ! @move_uploaded_file($this->file_temp, $upload_path.$this->file_name))
			{
		...
		if($this->resize) {
			$this->file_resize($this->upload_path . self::Org_Folder . '/' . $this->file_name);
		}
		/*
		...
		$this->saved_filename[] =  $this->file_name;
	...
	function file_resize($source_path)
	 { 

		if(isset($this->image_sizes) && is_array($this->image_sizes)){
			$ci =& get_instance();
			foreach($this->image_sizes as $key => $dimension) {
				@list($width, $height) = $dimension;
				$ci =& get_instance();
				$ci->load->library('image_lib');
				$config["source_image"] = $source_path;
				$pathinfo = pathinfo($this->file_name);
				$config['new_image'] = $this->upload_path . self::Mod_Folder . '/';;
				$config["width"] = $width;
				$config["height"] = 1;	
				$config['create_thumb'] = TRUE;
				$config['maintain_ratio'] = TRUE;
				$config['master_dim'] = 'width';
				$config['thumb_marker'] = '-'.implode('x',$dimension);
