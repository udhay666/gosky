        ...
        <div class="pageheader">
          ...
          <div class="page-bar  br-5">
            <ul class="page-breadcrumb">
              ...
              <li><a class="active" href="<?php echo site_url() ?>facility_type/facilitytype">Manage Facilities</a></li>
            </ul>
          </div>
        </div>
      </div>
        ...
        <form id="basicForm" class="form-horizontal" action="<?php echo site_url() ?>facility_type/<?php echo $action ?>" enctype="multipart/form-data" method="post">
           <div class="form-group">                     
             ...
             <div class="col-sm-2">
              ...
              <option value="room">Rooms Amenity</option>
      ...
      <table class="table table-custom dt-responsive" cellspacing="0" width="100%" id="table1">
          ...
          <tbody>
           ...
           <tr>
            ...
            <td>
            <?php if($facilitytype[$i]->facility_type=='room')
                 { 
                    echo 'Rooms Amenity';
                 }
                 else if($facilitytype[$i]->facility_type=='hotel')
                 { 
                    echo 'Hotel Amenity';
                 }
            ?></td>
            ...
            <td class="center">
             <a class="btn btn-success btn-rounded btn-sm btn-ef btn-ef-5 btn-ef-5a manage_facility_type_status"  data-url="<?php echo site_url(); ?>"
               data-id="<?php echo $facilitytype[$i]->id;  ?>" data-status="1"  title="Active"><i class="fa fa-check"></i> <span>Active</span></a>
             </td>
             <td>
               <a class="btn btn-danger btn-rounded btn-sm btn-ef btn-ef-5 btn-ef-5a manage_facility_type_status" data-url="<?php echo site_url(); ?>"
                data-id="<?php echo $facilitytype[$i]->id;  ?>" data-status="0" title="InActive"><i class="fa fa-times"></i> <span>Inactive</span></a>
              </td>
                <td>
                  ...
                  <td>
                  <a class="btn btn-info btn-rounded btn-sm btn-ef btn-ef-5 btn-ef-5a" href="<?php echo site_url(); ?>facility_type/view_facility_type_info/<?php echo $facilitytype[$i]->id;?>" title="Edit" ><i class="fa fa-times"></i> <span>Edit</span></a>                                 
                </td>                 
              </tr>             
              <?php } ?>
              <?php } ?>
  ...
  $(window).load(function(){
    ...
    $(".manage_facility_type_status").click(function(){ 
     ...
     if(confirm('Are you sure you want to '+ $message+' this Facility Type?')) {
       $.ajax({
        url: $siteUrl + 'facility_type/set_facility_type_status/',
        data: 'id='+$id+'&status='+$status,
        dataType: 'json',
        type: 'POST',
        success: function(data)
        { window.location.href=$siteUrl + 'facility_type/facilitytype/'; }
      });
     }
   });
