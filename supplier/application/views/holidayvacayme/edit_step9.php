...
?>
        ...
        <div class="tab-content">
          <div class="tab-pane active" id="step-9">
            <form action="<?php echo site_url() ?>holiday/update_all" method="post" class="step_form step9" steps="9" name="step9" role="form" id="submit_form" enctype="multipart/form-data" novalidate>
              <input type="hidden" name="steps" value="9">
              <input type="hidden" name="insert_id" id="insert_id" value="<?php echo $package_id ?>">
              <?php /**
              <div class="row border_row">
                <div class="form-group col-md-4">
                  <label class="strong" for="pkg_accomodation">Add Accommodation : </label><br>
                  <select name="pkg_accomodation[]" id="pkg_accomodation" class="select2_multiple form-control" multiple="multiple">
                    <?php foreach($accomodation_info as $acco){ ?>
                    <option value="<?php echo $acco->id ?>" <?php foreach($hotels as $ht) if($ht==$acco->id) echo 'selected' ?>><?php echo $acco->hotel_name ?></option>
                    <?php } ?>
                  </select>
                </div>
              </div>
          ...
          <ul class="pager wizard">
            <li class="previous">
              <a href="<?php echo str_replace('supplier/', '', site_url()) ?>holiday/preview_holiday/<?php echo $package_id ?>" target="_blank" class="fa fa-eye btn btn-default"> Preview</a>
            </li>
