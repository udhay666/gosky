      ...
      <div class="col-md-12">
        <section class="boxs">
          <div class="boxs-header dvd dvd-btm">
            <h1 class="custom-font"><strong>Hotel <?php echo $hotel_detail->hotel_name; ?><br/><small>Supplements Room Rate </strong>Calendar</small></h1>
            <ul class="controls">
             ...
             <div class="boxs-body">
               <form>
                ...
                <div class="form-group col-md-4">
                  <label class="strong" for="supplement_roomrate_type_id">Selete Supplement Room Rate Type : </label>
                  <select name="supplement_roomrate_type_id"  id="supplement_roomrate_type_id" class="form-control select2" required="required">
                    <option value="">Select Supplement Room Rate Type</option>
                    <?php for($i=0;$i<count($specialoffer_type);$i++){ ?>
                    <option value="<?php echo $specialoffer_type[$i]->id; ?>" <?php if($supplement_roomrate_type_id==$specialoffer_type[$i]->id){ echo "selected"; }?>><?php echo $specialoffer_type[$i]->type; ?></option>
                    <?php } ?>  
                  <option value="other" <?php if($supplement_roomrate_type_id=="other"){ echo "selected"; }?>>Other</option>                 
                  </select>
              </div>
...
$('.fc-button').click(function(){
  loadroomrates();
    ...
    {      
      ...
      $supplement_roomrate_type_id=$("#supplement_roomrate_type_id").val();  
       $.ajax({       
        url: siteUrl + 'supplements/get_room_rate_monthlist',
        data: 'room_id='+$room_id+'&hotel_code='+$hotel_code+'&contract_id='+$contract_id+'&startdate='+$startdate+'&enddate='+$enddate+'&supplement_roomrate_type_id='+$supplement_roomrate_type_id,
        dataType: 'json',    
        type: 'POST',
        beforeSend: function(){
    ...
    function loadroomrates($t='')
       ...
       $.ajax({       
        url: siteUrl + 'supplements/get_room_rate_list',
        data: 'room_id='+$room_id+'&hotel_code='+$hotel_code+'&contract_id='+$contract_id+'&month='+$getmonth+'&year='+$getyear+'&supplement_roomrate_type_id='+$supplement_roomrate_type_id,
        dataType: 'json',
        type: 'POST',  
         beforeSend: function(){
        $('#loadroomratesajax').modal('show'); 
...
$('#show_calender').on('click', function(){
 ...
 form.parsley().validate();
      ...
      {
        $.ajax({
                    url: siteUrl + 'supplements/get_room_rate_monthcalender',
                    data: form.serialize(),
                    dataType: 'json',
                    type: 'POST',
                    beforeSend: function(){
