...
public function manage_rate_status()
{
  ...
  $data['hotel_name']=$hotel_detail[0]->hotel_name;
     ...
     $data['normal_room_rate']=$this->sup_hotel_room_rates_list->check($dataarray1);
   ...
   $this->form_validation->set_rules('hotelname', 'Hotel Name', 'trim|required');   
    if($this->form_validation->run()==FALSE) {       
        ...
        $room_code = str_pad($room_code + 1, 10, 0, STR_PAD_LEFT);
         $child_agerange=array();
      for($i=0;$i<count($_POST['childageminlimit'])&&isset($_POST['childagemaxlimit']);$i++)
          {
             $child_agerange[$i]=$_POST['childageminlimit'][$i].'||'.$_POST['childagemaxlimit'][$i];
          }   
    $insertdata=array(
                    'supplier_id' =>$this->supplier_id,
                    ...
                    'total_extrabed' => $this->input->post('total_extrabed'),
                    'extrabedpermission' => $this->input->post('extrabedpermission'),
                    // 'childageminlimit' =>  implode(',',$this->input->post('childageminlimit')),
                    // 'childagemaxlimit' =>  implode(',',$this->input->post('childagemaxlimit')),
                    'child_agerange'=> json_encode($child_agerange),
