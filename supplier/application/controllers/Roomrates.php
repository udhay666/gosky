...
/* Meal Plan List */
  ...
  $data['mealplanlist'] =$mealplanlistnarr; 
    ...
    else
    {
       ...
       //print_r($data['sub_view']);
       $this->load->view('roomrate/add_rates',$data); 
  ...
  /* View The Existing Rate */
   public function view_existing_rate()
   {
     ...
     $data['existing_rate']= $this->sup_hotel_room_rates_list->get_roomrates_duplicate($this->supplier_id, $hotel_id, $contract,$room_id,$meal_plan,$market);
     //print_r($data); echo "tsting";exit;
 ...
 { 
  ...
  { 
      ...
      if($this->form_validation->run())
      {
        ...
        // if(isset($_POST['supplement_check']))
        // {
        //   for($i=0;$i<count($_POST['type_of_supplement'])&&isset($_POST['supplement_rate'])&&isset($_POST['supplement_compulsory_'.($i+1)]);$i++)
        //   {
        //     $supplement_arr[$i]=$_POST['supplement_compulsory_'.($i+1)].':'.$_POST['type_of_supplement'][$i].'||'.$_POST['supplement_rate'][$i];
        //   }

        //   $supplement=json_encode($supplement_arr);
        // }
        // else
        // {
        //   $supplement='';
        // }
        // print_r($supplement_arr); exit;
       ...
       else if(($rate_type=='PPPN' && !empty($_POST['adult_rate']))||($rate_type=='PRPN' && !empty($_POST['room_rate'])))
       {
        $insert_room_rates_list=array(    
           ...
           // $this->sup_hotel_room_rates_list->delete_room_rates_type_list($insert_room_rates_list);  
           $this->sup_hotel_room_rates->delete_room_rates_type($insert_room_rates_list);
           $this->sup_hotel_room_cancellation_rates->delete_room_cancellation_rates_type($insert_room_rates_list);
            // $this->sup_hotel_room_supplement_rates->delete_room_supplement_rates_type($insert_room_rates_list);
            ...
            // $this->sup_hotel_room_rates_list->delete_room_rates_list($insert_room_rates_list);  
           $this->sup_hotel_room_rates_list->update_delete_room_rates_list($insert_room_rates_list);
           $this->sup_hotel_room_rates->delete_room_rates($insert_room_rates_list);
           $this->sup_hotel_room_cancellation_rates->delete_room_cancellation_rates($insert_room_rates_list);
           // $this->sup_hotel_room_supplement_rates->delete_room_supplement_rates($insert_room_rates_list);
            ...
            {  
              ...
              // {
              //    $l=0;
              //     $insertsupplementdata=array();
              //     foreach ($_POST['type_of_supplement'] as $supplement_rate)
              //     { 
              //       $insertsupplementdata[$l]=array(    
              //           'sup_hotel_room_rates_list_id'=>$sup_hotel_room_rates_list_id,    
              //           'supplier_id' => $supplier_id,
              //          'sup_hotel_id' => $hotel_id,
              //           'hotel_code' => $hotel_code,
              ...
              //           'min_room_occupancy'=> $min_room_occupancy,
              //           'max_room_occupancy'=> $max_room_occupancy,  
              //           'type_of_supplement'=> $_POST['type_of_supplement'][$l],
              //           'supplement_rate'=> $_POST['supplement_rate'][$l],
              //           'supplement_compulsory'=>$_POST['supplement_compulsory_'.($l+1)],
              //           'date_time' => date('Y-m-d H:i:s'),
              //          );
              //       $this->sup_hotel_room_supplement_rates->insert($insertsupplementdata[$l]);
              //      $l++;
              //   }
              // }
...
$roomrates=$data['roomrates'] = $this->sup_hotel_room_rates->new_cal_get_roomrates_by_date($supplier_id,$hotel_id, $hotel_code, $room_id,$room_code,$contract_id,$market,$meal_plan,$from_date,$to_date);
 ...
 $dataarray=array('supplier_room_list_id'=>$room_id,'supplier_hotel_list_id'=>$hotel_id,'supplier_id'=>$this->supplier_id);
  $data['room_list'] =$this->supplier_room_list->check($dataarray);
    ...
    $data['result']=$this->sup_hotel_room_rates->get_roomrates($sup_hotel_room_rates_id, $room_code, $hotel_code,$this->supplier_id,$room_id,$contract_id);   
    $edit_room_rates='';
 ...
 public function update_room_rates_ind()
  ...
  $check=$this->sup_hotel_room_rates->get_roomrates_edit($hotel_id,$sup_hotel_room_rates_list_id,$sup_hotel_room_rates_id, $room_code, $hotel_code,$this->supplier_id,$sup_room_details_id,$contract_id,$market,$meal_plan);
  if($check!='')
  {
       ...
       { 
        ...
        'extra_bed_for_child_rate'=> $extra_bed_for_child_rate,                  
          ...
          $this->sup_hotel_room_rates->get_roomrates_update($hotel_id,$sup_hotel_room_rates_list_id,$sup_hotel_room_rates_id, $room_code, $hotel_code,$this->supplier_id,$sup_room_details_id,$contract_id,$market,$meal_plan,$updatadata);
  ...
  public function update_room_rates_ind_old1()
  {    
    $hotel_id =$_POST['hotel_id'];
    $sup_room_details_id = $_POST['sup_room_details_id'];    
    $sup_hotel_room_rates_list_id = $_POST['sup_hotel_room_rates_list_id'];    
   ...
   $min_room_occupancy=isset($_POST['min_room_occupancy'])?$_POST['min_room_occupancy']:'';
   $max_room_occupancy=isset($_POST['max_room_occupancy'])?$_POST['max_room_occupancy']:'';    
    $this->sup_hotel_room_cancellation_rates->delete_room_cancellation_data($this->supplier_id,$hotel_id,$hotel_code,$sup_room_details_id,$room_code,$contract_id,$meal_plan,$market,$room_avail_date,$sup_hotel_room_rates_list_id);
    // $this->sup_hotel_room_supplement_rates->delete_room_supplement_data($this->supplier_id,$hotel_id,$hotel_code,$sup_room_details_id,$room_code,$contract_id,$meal_plan,$market,$room_avail_date,$sup_hotel_room_rates_list_id);
 // if(isset($_POST['supplement_check']))
 //  {
 //     $l=0; 
 //     $insertsupplementdata=array();
 ...
 $check=$this->Roomrates_Model->get_roomrates_edit($sup_hotel_room_rates_id, $room_code, $hotel_code,$supplier_id,$sup_room_details_id);
if($check!=''){
 if($_POST['rate_type']=='PRPN'){ 
    $updatadata=array(        
            'room_rate'=> $_POST['room_rate'],
          ...
          $this->Roomrates_Model->get_roomrates_update($sup_hotel_room_rates_id, $room_code, $hotel_code,$sup_room_details_id,$supplier_id,$updatadata);
          echo json_encode(array('success' => 'true'));
      }
  else if($_POST['rate_type']=='PPPN'){ 
    $updatadata=array(        
...
{   
    ...
    $calender_data = $this->sup_hotel_room_rates->get_roomallotment_by_date($room_id,$hotel_code,$startdate, $enddate,$contract_id,"");   
   $calendar=array();
   $calendar_date=array();  
    list($calendar,$calendar_date)=$this->roomrates_calendar($calender_data);   
    if (!empty($calender_data)) {     
   ...
   for($i=0;$i<count($calender_data)&&!empty($calender_data[0]);$i++,$k++)
  {
       ...
       {         
         ...
         "Adult Rate(Extra Bed) : ".$calender_data[$i]->extra_bed_for_adults_rate.
          ...
          "<br>Market : ".$calender_data[$i]->market.$status."</small>";
