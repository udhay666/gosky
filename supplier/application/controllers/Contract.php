    ...
    if($this->form_validation->run()==FALSE) {     
      ...
      $errors_msg="Try After Sometimes....";
       $this->session->set_flashdata('errors_msg',$errors_msg);
        ...
        $market_avail='';
         if(isset($_POST['all_market'])){
             $market_avail="All Markets";
         }
         else
         {
            $market_avail=implode('||',$this->input->post('market_avail'));
          }
...
{   
$data['contract_number']=$contract_number=isset($_GET['contract_number']) ? $_GET['contract_number'] : '';
$data['hotel_code']=$hotel_code=isset($_GET['hotel_code']) ? $_GET['hotel_code'] : '';
$data['status']=$status=isset($_GET['status']) ? $_GET['status'] : '';     
$data['status1']=$status1=isset($_GET['status1']) ? $_GET['status1'] : '';
$data['from_date']=$from_date=isset($_GET['from_date']) ? $_GET['from_date'] : '';
$data['to_date']=$to_date=isset($_GET['to_date']) ? $_GET['to_date'] : '';
$data['created_fromdate']=$created_fromdate=isset($_GET['created_fromdate']) ? $_GET['created_fromdate'] : '';
$data['created_todate']=$created_todate=isset($_GET['created_todate']) ? $_GET['created_todate'] : '';

  $data['contract_list'] =$this->sup_contract->getcontractlist($this->supplier_id,$contract_number,$hotel_code,$status,$status1,$from_date,$to_date,$created_fromdate,$created_todate); 
...
public function edit_step3_old() {
    $data['id'] = $id = $_GET['id'];
    $data['contract_id']= $data['id'] = $contract_id = $_GET['id'];
    $dataarray=array('contract_id'=>$contract_id,'supplier_id'=>$this->supplier_id);
    $data['contract_info'] =$this->sup_contract->check($dataarray);
...
public function update_step3_old($id='')
{
    $data['contract_id']= $data['id'] = $contract_id = $id;
    $dataarray=array('contract_id'=>$contract_id,'supplier_id'=>$this->supplier_id);
    $data['contract_info'] =$this->sup_contract->check($dataarray);
...
public function edit_step4() {
    ...
    $dataarray=array('contract_id'=>$contract_id,'supplier_id'=>$this->supplier_id);
    $data['contract_info'] =$this->sup_contract->check($dataarray);
    $data['payment_credentials_info']=$this->sup_contract_payment_condition->check($dataarray);
  // echo '<pre>';  print_r($data['payment_credentials_info'][0]); exit;
    if(empty($data['contract_info']))
    {
      redirect('contract/contract_list','refresh');
    }   
...
public function update_step4($id='')
{
    ...
    $dataarray=array('contract_id'=>$contract_id,'supplier_id'=>$this->supplier_id);
    $data['contract_info'] =$this->sup_contract->check($dataarray);
    $check=$this->sup_contract_payment_condition->check($dataarray);
   ...
   if(!empty($check))
    {
      $this->sup_contract_payment_condition->deletepaymentcondition($contract_id,$this->supplier_id);
    }
...
public function viewcontractcomments()
{
  ...
  {
     if(isset($_POST['contract_id'])&&isset($_POST['season_name'])){
      $dataarray=array('contract_id'=>$_POST['contract_id'],'supplier_id'=>$this->supplier_id);
      $check=$this->sup_contract->check($dataarray);
      if($check==''){
        redirect('contract/contract_list','refresh'); 
         ...
         $season_code = $this->sup_contract_season->get_last_season_code();
          $season_code = str_pad($season_code + 1, 10, 0, STR_PAD_LEFT);  
    $insertdata=array(
                      'supplier_id'=>$this->supplier_id,
                      'contract_id'=>$_POST['contract_id'],
