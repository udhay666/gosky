...
public function add_holiday() {
    ...
    $data['currency'] = $this->Currency->get_active_currency();
...
public function save_holiday_step1() {
    ...
    if($this->form_validation->run()==FALSE) {
        ...
        if($check_insert == ''){
            ...
            // echo $this->db->last_query();exit;
             $supplier = $this->supplier_hotel_list->get_sup_name($data['supplier_id']);
             // print_r($insert_id);exit;
            $data_hotel = array(
                'supplier_name' => $supplier,
                'hotel_name' => $data['package_title'],
                'hotel_city' => $data['country'],
                'action' => 'Inserted',
                'module' => 'Holiday',
            );
            // echo '<pre>11';print_r($data_hotel);exit;
            $this->Email_Model->admin_conformation($data_hotel);
            // $this->Email_Model->email_testing();
...
public function update_step1($package_id){
    ...
    if($this->form_validation->run()==FALSE) {
        ...
        $supplier = $this->supplier_hotel_list->get_sup_name($data['supplier_id']);
            ...
            // echo '<pre>11';print_r($data_hotel);exit;
            $this->Email_Model->admin_conformation($data_holiday);
...
public function delete(){
    $id = $_GET['id'];
    $this->load->model('Holiday_packages');
    $this->Holiday_packages->deleteHolidaypackage($id);
    $this->Holiday_packages->deleteHolidayactivity($id);
    $this->Holiday_packages->deleteHolidaymeeting($id);
    redirect('holiday/holiday_list', 'refresh');
