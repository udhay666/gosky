...
class Sup_hotel_room_rates_list extends MY_Model {

    protected $_table_name = 'sup_hotel_room_rates_list';
    ...
    function check($array=NULL) {
     ...
     public function get_roomrates_duplicate($supplier_id, $sup_hotel_id, $contract_id,$sup_room_details_id,$meal_plan='',$market) {
        $this->db->select('*');
...
function delete_room_rates_list_old($hotel_id, $room_id, $startdate,$enddate,$contract_id,$market,$meal_plan) {
        ...
        $this->db->where('market', $market);
         $this->db->where('meal_plan', $meal_plan);
        $this->db->delete('sup_hotel_room_rates_list');
    }
