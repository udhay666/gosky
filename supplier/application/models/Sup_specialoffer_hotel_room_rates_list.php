...
class Sup_specialoffer_hotel_room_rates_list extends MY_Model {

    protected $_table_name = 'sup_specialoffer_hotel_room_rates_list';
    protected $_primary_key = 'sup_hotel_room_rates_list_id';
    protected $_primary_filter = 'intval';
    protected $_order_by = "sup_hotel_room_rates_list_id desc";
    ...
    function check($array=NULL) {
     ...
     public function get_roomrates_duplicate($supplier_id, $sup_hotel_id, $contract_id,$sup_room_details_id,$meal_plan='',$market,$specialoffer_id) {
        $this->db->select('*');
        ...
        $this->db->order_by('sup_hotel_room_rates_list_id', 'desc');        
        $query = $this->db->get();      
        if ($query->num_rows() > 0) {
            return $query->result();
        } else {
...
function delete_room_rates_list_old($hotel_id, $room_id, $startdate,$enddate,$contract_id,$market,$meal_plan,$specialoffer_id,$specialoffer_type) {
        ...
        $this->db->delete('sup_specialoffer_hotel_room_rates_list');
    }

 public function get_roomrate_duplicate($supplier_id, $sup_hotel_room_rates_list_id)
      {
        $this->db->select('*');
        $this->db->from($this->_table_name);
        $this->db->where('supplier_id', $supplier_id);
