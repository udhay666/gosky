...
$( "#fromCity" ).autocomplete({	
    source: siteUrl+domain+"/airport_autolist?exclude="+$("input[name='destination']").val(),		
    ...
    select: function( event, ui ) {
        ...
        $( "#toCity" ).autocomplete( "option", "source", siteUrl+domain+"/airport_autolist?exclude="+$("input[name='origin']").val() );
    ...
    $( "#toCity" ).autocomplete({			
    source: siteUrl+domain+"/airport_autolist?exclude="+$("input[name='origin']").val(),
    minLength: 2,
    autoFocus: true,               
    select: function( event, ui ) {
        if( ui.item.id == '' ) return false;
        $("input[name='destination']").val(ui.item.id);
        $( "#fromCity" ).autocomplete( "option", "source", siteUrl+domain+"/airport_autolist?exclude="+$("input[name='destination']").val() );
