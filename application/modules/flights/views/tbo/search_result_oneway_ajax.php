...
<?php for ($i = 0; $i < count($result); $i++) { 
    ...
    if ($stops==0) { $route = "Direct";}else{$route = "Stop 1";}
            ...
            <div class="row no-gutter">
                <main class="col-md-9 col-sm-12 item-flight-left"><br>              
                    <div class="row row-trip no-gutter">
                        ...
                        <div class="info-stops">
                            <p class="place-wrap">
                            <strong class="city" data-toggle="tooltip" title="" data-original-title="<?php echo current($operating_cityname_o); ?>"><?php echo current($operating_cityname_o); ?> </strong> <span class="code " data-toggle="tooltip" title="" data-original-title="">(<?php echo current($operating_airportname_o);?>)</span>
                            ...
                            <p class="place-wrap"><span class="code " data-toggle="tooltip" title="" data-original-title="">(<?php echo end($operating_airportname_d);  ?>)</span> <strong class="city" data-toggle="tooltip" title="" data-original-title="<?php echo end($operating_cityname_d); ?>"><?php echo end($operating_cityname_d); ?></strong>  
                    ...
                    <aside class="col-md-3  col-sm-4">
                        ...
                        <div class="info-icons">
                            ...
                            <?php } ?>
                            <?php if(!empty($faretype)){ ?>
                            <!-- <span class="icon icon-layover" data-toggle="tooltip" title="" data-original-title="groupfare"><?php //echo $faretype.'--'.$flightno; ?></span> -->
                            <?php } ?>
                ...
                <div class="info-buy">
                    ...
                    <form name="<?php echo "flight_segmentform_".$i.$operating_airlinecode[0].$total_amount; ?>" id="<?php echo "flight_segmentform_".$i.$operating_airlinecode[0].$total_amount; ?>">
                        ...
                        <?php $morefare = $this->Tbo_Model->morefare($flightno,$session_id,$uniquerefno);
                        //echo"<pre>";print_r($morefare);exit;
                        ?>
                        <?php //if(($morefare)){ ?>
                        <!--<p><a onclick="ShowFares('<?php echo 'ticketid0123'.$i.$operating_airlinecode[0].round($total_amount); ?>')" class="btn btn-block btn-warning btn-details">See Fares <b class="caret"></b></a></p>-->
                        <?php //}else{ ?>
                        <p><?php $urlstring=base64_encode('tbo/'.$search_id.'/'.$segmentkey); ?></p>
                        <p><a href="<?= site_url() ?>flights/itinerary/<?= $urlstring ?>" target="_blank" class="btn btn-block btn-warning">Book Now</a></p>
                        <?php //} ?>
                    </form>
            ...
            <section class="tab-pane fade in active" id="<?php echo 'flight_menu-'.$i.$operating_airlinecode[0].round($total_amount); ?>">
                <article class="ticket-detail">
                    <header class="heading-ticket">
                        ...
                        <h4 class="title">Flight from <strong><?php echo current($operating_cityname_o); ?></strong> To <strong><?php echo end($operating_cityname_d); ?></strong> On <strong><?php echo date('l, d F Y', strtotime($operating_deptime[0])); ?></strong></h4>
                    </header>
                    ...
                    <div class="row row-trip no-gutter">
                        ...
                        <aside class="col-sm-7 col-xs-12">
                            <div class="info-flight-way">
                                ...
                                <br><?php $dep = explode('T', $operating_deptime[$j]); $dep[1] = substr($dep[1], 0, -3); echo date('dS M',strtotime($dep[0]));?> <strong><?php //echo date('H:i',strtotime($deptime[1]));
                                ...
                                ?></strong>
                                    <br><?php echo $this->Tbo_Model->airportname($operating_airportname_o[$j]); ?>
                                     <br>Terminal: <?php if ($operating_terminal_o[$j] != '') echo $operating_terminal_o[$j]; ?></p>
                                <p class="way-wrap">
                                <span class="txt-time">
                                    <?php //echo $durationd; ?>
                                ...
                                <p class="place-wrap-to">(<?php echo $operating_airportname_d[$j];?>) <strong><?php echo $operating_cityname_d[$j]; ?></strong>
                                   ...
                                   $arr = explode('T', $operating_arritime[$j]); $arr[1] = substr($arr[1], 0, -3);
                                    echo $arr[1];
                                    ?></strong> <?php echo date('dS M',strtotime($arr[0]));?> <span data-toggle="tooltip" title="" class="badge" data-original-title="Arrival next day">+1</span>
                                    <br><?php echo $this->Tbo_Model->airportname($operating_airportname_d[$j]); ?> <br><?php if ($operating_terminal_d[$j] != '') echo 'Terminal : ' . $operating_terminal_d[$j];  ?></p>
                            </div>
                        </aside> <!-- col // -->
                        <aside class="col-sm-3 col-xs-12 hidden-xs"> 
                            <ul class="list-default">
                                ...
                                <li>Cabin Class: <span> <?php if($cabinClass==1){echo "Economy";} ?></span></li>
                            </ul>
                        </aside><!-- col // -->
                    </div> <!-- row-trip // -->
