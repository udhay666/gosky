...
class Corporate extends MX_Controller {
    ...
    private function agent_logged(){
        if (!$this->session->has_userdata('corporate_logged_in') && !$this->session->has_userdata('user_logged_in')){
    ...
    public function login() {
        //print $this->session->has_userdata('corporate_logged_in');
        if ($this->session->has_userdata('corporate_logged_in')){
            //redirect('corporate/dashboard', 'refresh');
            redirect('corporate/my_bookings', 'refresh');
        }            
        // $this->load->view('corporate/account/login');
        redirect('home/index', 'refresh');
    }
    ...
    public function dashboard() {
        $this->unset_all();
        $this->agent_logged();
        $agent_info = $this->Corporate_Model->getAgentInfo();
    ...
    public function manage_users() {
        ...
        $this->agent_logged();      
        $data['agent_info'] = $this->Corporate_Model->get_agent_list(); 
        //echo $this->db->last_query(); exit;  
        
       $this->load->view('corporate/agent_manager',$data);
    }

    public function deposit_approve($acc_id,$agent_no) {
        //print $acc_id."test".$agent_no;exit;
        $available = '0';
        $deposite = '0';
        $status = 'Accepted';
        $data['agentno'] = $agent_no;
        $data['depositno'] = $acc_id;
        $balance = $this->Corporate_Model->get_agent_available_balance($agent_no,$acc_id);
        //echo $this->db->last_query();exit;
        $deposite = $this->Corporate_Model->get_approved_amount($acc_id);
        //echo $this->db->last_query();exit;
        ...
        $data['remarks'] = $deposite->remarks;
        $data['bank'] = $deposite->bank;
        $data['branch'] = $deposite->branch;
        $data['city'] = $deposite->city;

        $data['available_balance'] = $balance;
        $this->load->view('corporate/approve_page', $data);
    }
    public function deposit_decline($acc_id,$agent_no) {
        $deposit = $this->Corporate_Model->agent_decline_deposit($acc_id,$agent_no);
        $agent_info = $this->Corporate_Model->get_agency_name($agent_no);
    $this->Email_Model->deposit_decline_mail($agent_no, $agent_info->agency_name, $agent_info->agent_email);
        ...
        redirect('corporate/view_account_stmt', 'refresh');
    }
    public function approve_amount() {
        //echo "here";exit;
        //echo '<pre>'; print(; exit;
        ...
        $update = $this->Corporate_Model->update_deposit_status($dep_amt, $depositno, $total);
$agent_info = $this->Corporate_Model->get_agency_name($agent_no);
        //$get_sum_of_deposits = $this->B2b_Model->get_sum_of_deposits($agent_no);
        //$deposit_amount = $get_sum_of_deposits;
     $this->Corporate_Model->deposit_approve_mail($agent_no, $total, $dep_amt, $agent_info->agency_name, $agent_info->agent_email);
    ...
    public function corporate_register(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $email_check = $this->Corporate_Model->check_email_availability($insertdata['agent_email']);
            if ($email_check != '' || !empty($email_check)) {
                ...
                $agent_id = $this->Corporate_Model->add_agent($insertdata);
                if($agent_id) {
                    $this->Corporate_Model->update_agent($agent_id,$upd_data);
        ...
        if($status == 'success') {
            ...
            $data['country_list'] = $this->Corporate_Model->get_country_list();
            // $data['currency_list'] = $this->Corporate_Model->get_currency_list();
            $data = array_merge($data, $insertdata);
            $this->load->view('corporate/account/corporate_register', $data);
        }
    }
   ...
   public function view_account_stmt($agent_id='')
    {
        ...
        $data['agent_info'] = $this->Corporate_Model->get_agent_info_by_id($agent_id);       
        $data['agent_acc_summary'] = $this->Corporate_Model->get_acc_summary($agent_id);
        //echo $this->db->last_query();exit;
        $this->load->view('corporate/view_account_stmt',$data);
    }
    ...
    function update_profile() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($this->Corporate_Model->update_agent($agent_id,$upd_data)) {
        ...
        if($status == 'success') {
            ...
            $data['country_list'] = $this->Corporate_Model->get_country_list();
            $data['agent_info'] = $this->Corporate_Model->getAgentInfo($agent_id);
            // $data['currency_list'] = $this->Corporate_Model->get_currency_list();
            $data = array_merge($data, $upd_data);
            $this->load->view('corporate/account/view_profile', $data);
        }
    }
    ...
    function corporate_login() {
        ...
        if ($this->form_validation->run() !== FALSE) {
            ...
            $res = $this->Corporate_Model->validate_credentials($loginEmailId, $loginPassword);
            ...
            if ($res != '') {
                ...
                $this->Corporate_Model->insert_login_activity();
               // redirect('corporate/dashboard', 'refresh');
                redirect('corporate/my_bookings', 'refresh');
            } else {
...
/*
        ...
        $data['admin_priviliges']=$this->Corporate_Model->get_admin_privilages();
        ...
        {*/
            ...
            $email_check = $this->Corporate_Model->check_subemail_availability($admin_email);
            ...
            {
                if($inert_id=$this->Corporate_Model->add_admin_user($admin_email,$admin_password,$title,$first_name,$middle_name,$last_name,$mobile_no,$address,$pin_code,$city,$state,$country,$role_id))
                {
                    $email_data = array(
    ...
    public function logout() {
        ...
        $this->session->unset_userdata('corporate_logged_in');
    ...
    function corporatesendOtp(){
        ...
        if ($this->form_validation->run() !== FALSE) {
            $res = $this->Corporate_Model->validate_credentials_otp($loginEmailId);
            // echo $this->db->last_query();
              // echo '<pre>';print_r($res);exit;
            if ($res !== false) {
                if(is_numeric($loginEmailId)){
                ...
                // echo $otpnum;//exit;
                $otp = $this->Corporate_Model->insertOtp($loginEmailId,$otpnum);
                // echo $this->db->last_query();
                // echo $otp;exit;
                if($otp != ''){
                    $status = 'success';
    ...
    function corporateotpLogin(){
        ...
        if ($this->form_validation->run() !== FALSE) {
            $res = $this->Corporate_Model->corporate_validate_otp($loginEmailId,$otp_number);
            // echo $this->db->last_query();
             // echo '<pre/>';print_r($res);exit;
            if ($res !== false) {
                $message = 'Otp matched!';
                ...
                $this->Corporate_Model->removeOTP($loginEmailId);
                $this->Corporate_Model->insert_login_activity();
                // exit;
    ...
    public function forgot_password() {
        ...
        if ($this->form_validation->run() !== FALSE) {
            ...
            $getpassword = $this->Corporate_Model->get_forgot_password($loginEmailId);
            if($getpassword == '') {
                $status = 'fail';
                $message = "Sorry !!! ".$loginEmailId." is not registered";
            } else {
                ...
                if($this->Corporate_Model->update_agent_activation_key($otp, $agent_no)){
                    // echo $this->db->last_query();exit;
    ...
    function change_password() {
        ...
        $agent_info = $this->Corporate_Model->getAgentInfo($agent_id);
        if ($this->form_validation->run() == FALSE) {
            $data['message'] = validation_errors();
            $status = 'fail';
        } else {
            if (md5($this->input->post('current_password')) == $agent_info->agent_password) {
                if ($this->input->post('password') == $this->input->post('passconf')) {
                    ...
                    if ($this->Corporate_Model->update_password($agent_id, $password)) {
    ...
    function change_otp_password() {
        ...
        $message = '';
        $agent_id = $this->session->agent_id;
        $agent_info = $this->Corporate_Model->getAgentInfoByID($agent_id);
        // print_r($agent_info);
        if ($this->form_validation->run() == FALSE) {
            $message = validation_errors();
            $status = 'fail';
    ...
    function restore_password() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $password = md5($this->input->post('password'));
            if ($this->Corporate_Model->restore_password($agent_email, $password)) {
                // echo $this->db->last_query();
                $data_email = array(
                    // 'name' => 'Corporate',
                    'email' => $agent_email,
    ...
    public function my_bookings($booking_type='') {
         $this->agent_logged();
        // print_r($this->session->corporate_logged_in);exit;
        $data['agent_id'] = $agent_id = $this->session->corporate_logged_in;
        ...
        if($booking_type == 'flights'){
            $module = 'flights';
            $data['flight_booking_summary'] = $this->Corporate_Model->get_b2b_flight_booking_summary($fromdate,$todate,$uniqueRefNo,$status,$email,$mobile);

        }else if($booking_type == 'hotels') {
            $data['hotel_booking_summary'] = $this->Corporate_Model->get_b2b_hotel_booking_summary($fromdate,$todate,$uniqueRefNo,$status,$email,$mobile);
            //echo $this->db->last_query();
        }else if($booking_type == 'bus') {
             $module = 'bus';
             $data['bus_booking_summary'] = $this->Corporate_Model->get_b2b_bus_booking_summary($fromdate,$todate,$uniqueRefNo,$status,$email,$mobile);
           
        }else if($booking_type == 'cabs') {
              $module = 'cabs';

            $data['car_booking_summary'] = $this->Corporate_Model->get_b2b_car_booking_summary($fromdate,$todate,$uniqueRefNo,$status,$email,$mobile);
            //echo $this->db->last_query();exit;
           
        }else if($booking_type == 'transfer') {
              $module = 'transfer';
            $data['transfer_booking_summary'] = $this->Corporate_Model->get_b2b_transfer_booking_summary($fromdate,$todate,$uniqueRefNo,$status,$email,$mobile);
            //echo $this->db->last_query();exit;
        }else if($booking_type == 'activity') {
              $module = 'activity';
            $data['activity_booking_summary'] = $this->Corporate_Model->get_b2b_activity_booking_summary($fromdate,$todate,$uniqueRefNo,$status,$email,$mobile);
            //echo $this->db->last_query();exit;
        }else{
            $data['hotel_booking_summary'] = $this->Corporate_Model->get_b2b_hotel_booking_summary($fromdate,$todate,$uniqueRefNo,$status,$email,$mobile);
        }
        ...
        $this->load->view('corporate/bookings/'.$module, $data);
    }

    public function markup_management() {
        $this->agent_logged();
        $data['agent_no'] = $agent_no = $this->session->agent_no;
        $data['agent_markup_manager'] = $this->Corporate_Model->get_agent_markup_manager($agent_no);
    ...
    public function deposit_management() {
        ...
        $data['agent_info'] = $this->Corporate_Model->getAgentInfo($agent_id);
        $data['agent_deposit_details']  =$this->Corporate_Model->get_agent_deposit_details($agent_id);

        //echo '<pre/>';print_r($data);exit;
        $this->load->view('corporate/deposit/deposit_request', $data);
    }
    ...
    public function deposit_summary() {
        ...
        //echo '<pre/>';print_r($data);exit;
        $this->load->view('corporate/deposit/view_deposit_summary', $data);
    }
    ...
    function add_markup() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->Corporate_Model->add_markup($agent_no,$service_type,$markup,$markup_process)) {
    ...
    function deposit_request() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->Corporate_Model->add_deposit_request($agent_id,$agent_no,$amount,$value_date,$transaction_mode,$bank,$branch,$city,$transaction_id,$remarks)) {
                // echo $this->db->last_query();exit;
            ...
            /*$agent_info = $this->Corporate_Model->getAgentInfo($this->session->userdata('agent_id'));
            ...
            $this->session->set_flashdata('message', $message);          
            redirect('corporate/deposit_management', 'refresh');
        }
    }
