...
<?php
    ...
    // echo '<pre>'; print_r($this->session->userdata('agent_logged_in'));exit;
    if (!empty($roomDetails->image)) {
        $images = explode(',', $roomDetails->image);
        $gttd = $images[0];
        // print_r($gttd);exit;
    } else {
        $gttd = false;
...
/*    // $session_data = $this->session->userdata('hotel_search_data');
    $IsPassportMandatory = $this->session->IsPassportMandatory; 
    $IsPANMandatory = $this->session->IsPANMandatory; 
...
<section class="section-padding inner-page">
  <div class="container">
    ...
    <form name="booking" method="POST" action="<?php echo site_url(); ?>hotels/confirm_reservation?callBackId=<?php echo base64_encode($roomDetails->api); ?>&hotelCode=<?php echo $roomDetails->hotel_code; ?>&searchId=<?php echo $roomDetails->search_id; ?>&sessionId=<?php echo $roomDetails->session_id; ?>" data-parsley-validate >
            ...
            <div class="white-container">
              <div class="row">
                <div class="col-lg-4 col-md-4">
                  <?php if ($gttd) { ?>
                  <img src="<?php echo base_url().'supplier/'. $gttd; ?>" width="100%" height="170" alt="<?php echo $roomDetails->hotel_name; ?>" title="<?php echo $roomDetails->hotel_name; ?>" border="0" style="height: 170px;">
                  <?php } else { ?>
          ...
          <div class="itinerary-container box-shadow mb-3">
            ...
            <div class="white-container">
              <table class="rooms-table row2">
                <tbody>
                  <tr>
                    <td>
                      ...
                      <table>
                        <tr>
                          <td style="font-size: 40px"><?php echo date('d', $checkIn) ?></td>
                          <td style="font-size: 12px;padding-left: 15px;">
                            <?php echo date('l', $checkIn) ?><br><?php echo $checkIn ?>
                          </td>
                        </tr>
                      </table>
                    </td>
                    ...
                    <td>
                      ...
                      <table>
                        <tr>
                          <td style="font-size: 40px"><?php echo date('d', $checkOut) ?></td>
                          <td style="font-size: 12px;padding-left: 15px;">
                            <?php echo date('l', $checkOut) ?><br><?php echo $checkOut ?>
                          </td>
                        </tr>
                      </table>
                    </td>
               ...
               <?php
                        $room_type=explode(',',$roomDetails->room_name_desc);
                        $room_name='';
                        for($r=0;$r<count($room_type);$r++){
                        $room_no=$r+1;
                        $room_name.= 'Room '.$room_no.' : '.$room_type[$r].'<br>';
                        } ?>
              <div class="border-line"></div>
              <table class="row2 rooms-table2">
                <tbody>
                  ...
                  <tr>  
                    <td>Room Name <?php echo $room_name ?></td>
                  </tr>
                  <tr> 
                  ...
                  <tr>
                    <td>  
                    Number of Night <?php echo $nights; ?> Night(s)</td></tr>
                    <tr>
                    <td>  
                    Number of Room  <?php echo $rooms; ?> Room(s)</td></tr> 
                    <tr>
                    <td> Number of Adult <?php echo $adults_count ?></td></tr> 
                    <tr>
                    <td> Number of Child <?php echo $childs_count ?>
                    </td>
                  </tr> 
                </tbody>
              </table>
            ...
            <div class="white-container">
              ...
              <!-- <div class="row">
                <div class="col-sm-12 text-right">
                  <div class="border-line"></div>
                  <?php if ($this->session->userdata('agent_logged_in')  || $this->session->has_userdata('corporate_sub_logged_in') ) {
                  ...
                  <?php 
                    ...
                    $agent_no = $this->session->agent_no;
                     ...
                     if($this->session->has_userdata('agent_logged_in') || 
                      $this->session->has_userdata('corporate_sub_logged_in')){
                      $available_balance = $this->Hotelcrs_Model->get_agent_available_balance();
                    // echo $this->db->last_query();
                     $tot = $seat_price - $agent_markup;                    
