...
class Hotels_Model extends CI_Model {
  ...
  public function TotalSearchResults($sess_id, $minPrice = '', $maxPrice = '',$tbolistc='', $starRating = '', $hotelName = '', $location = '') {
    //$this->db->group_by('p.hotel_name_unique');

   /* if ($tbolistc != '') {
     // $tbolistcs = explode(',', $tbolistc);
     $team= $this->db->query("select * from hotel_city where LocationCategoryCode IN(".$tbolistc.")")->result();
     //print_r($team);
 ...
 // $session_data = $this->session->userdata('hotel_search_data');
  ...
  // $this->db->where('city_id',$cityid);
   ...
   /* if ($tbolistc != '') {
      $tbolistcs = explode(',', $tbolistc);
      for($i=0;$i<count($tbolistcs);$i++){
      $this->db->like('t.address', $tbolistcs[$i]);
    }
  }*/
  ...
  if ($tbolistc != '') {
    // $tbolistcs = explode(',', $tbolistc);
    $team= $this->db->query("select * from hotel_city where LocationCategoryCode IN(".$tbolistc.")")->result_array();
    
   $u="";
    foreach($team as $teams){
    $u.=$teams["TBOHotelCode"].",";

    }
    $m= substr_replace($u ,"", -1);
  
   $this->db->where("t.hotel_code in ($m)");
  ...
  public function all_fetch_search_result($sess_id, $offset, $perPage, $minPrice = '', $maxPrice = '',$tbolistc='', $starRating = '', $hotelName = '', $location = '', $sortBy = '', $order = '') {
    $this->db->group_by('t.hotel_code');
 //$this->db->group_by('p.hotel_name_unique');
...
public function get_selected_hotel($hotelCode, $session_id, $searchId) {
  $this->db->select('t.*,p.*');
  $this->db->from('hotel_search_result t');
  $this->db->join('api_permanent_hotels p', 't.hotel_code = p.hotel_code AND t.api = p.api','right');
  $this->db->where('t.hotel_code', $hotelCode);
...
public function insert_tbostaticcity($data)
{
  $this->db->insert('hotel_city', $data);
